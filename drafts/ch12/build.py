...
def derived():
    ...
    table("rewrite-table", ["Generation", "Word"], [[i,s] for i,s in enumerate(r["rewrite"])], "rl")
    table("graph-table", ["ID", "Operation", "Input IDs", "Owner"],
          [[v["id"],v["op"],",".join(map(str,v["inputs"])),v["parameter"] or "--"] for v in r["graph"]], "rlll")
    table("cost-table", ["Depth", "Calls", "Nodes", "Shared params", "Separate params"],
          [[v[k] for k in ["depth","affine_calls","nodes","parameters","independent_parameters"]] for v in r["costs"]], "rrrrr")
    plot("cost-plot",r["costs"],"depth",["parameters","independent_parameters"])
