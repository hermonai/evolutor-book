...
def derived():
    ...
    table(
        "routing",
        ["Policy", "Useful MACs", "Executed MACs", "Dropped"],
        [
            [
                v["name"],
                v["useful_expert_macs"],
                v["executed_expert_macs"],
                v["dropped_assignments"],
            ]
            for v in r["routing"]
        ],
        "lrrr",
    ...
    table(
        "buffer",
        ["Events", "Capacity", "Actual", "Potential", "Amortized"],
        [
            [
                v[k]
                for k in (
    ...
    plot("roofline-plot", r["roofline"], "intensity", ["throughput"])
    buffer_lines = []
    for key, style in [
        ("capacity", "Blue,thick,const plot"),
        ("actual", "Red,dashed,mark=*"),
    ]:
        points = " ".join(
            f"({v['size']},{v[key]})" for v in r["buffer"][:8]
        )
        if key == "capacity":
            points = "(0,0) " + points
        buffer_lines.append(
        ...
        buffer_lines.append(
            ...
            + (
                "Allocated slots"
                if key == "capacity"
                else "Moves for this append"
            )
            + "}"
        )
    files[BUILD / "buffer-plot.tex"] = "\n".join(buffer_lines) + "\n"
