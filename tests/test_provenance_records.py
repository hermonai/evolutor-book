...
HISTORICAL = ROOT / "historical" / "pre-reboot"
...
def test_checked_in_provenance_records_are_valid() -> None:
    records = sorted((HISTORICAL / "research" / "provenance").glob("*.json"))
    assert records
...
def test_claim_requires_immutable_source_identity() -> None:
    record_path = HISTORICAL / "research" / "provenance" / "claim-g012.json"
